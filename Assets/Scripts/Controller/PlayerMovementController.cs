...
public class PlayerMovementController : MonoBehaviour
{
    ...
    private CharacterController m_charCtrllor;
    ...
    void Start()
    {
        m_charCtrllor = this.GetComponent<CharacterController>();
    }
    ...
    void Update()
    {
        if(Input.touchCount > 0)
        {
            ...
            if (currentTouch.phase == TouchPhase.Moved)
            {
                ...
                m_rotateAngle = Mathf.Atan2((m_dragPos.y - m_beginPos.y),
                    (m_dragPos.x - m_beginPos.x)) * Mathf.Rad2Deg - 90f;

                if (m_rotateAngle <= 100)
                    m_rotateAngle -= 360;
            ...
            m_charCtrllor.Move(m_moveDirection);
